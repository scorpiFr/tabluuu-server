...
const { sequelize, Staticmenu } = require("../models");
...
async function countMenus(etablissementId) {
  try {
    const [results, metadata] = await sequelize.query(
      "SELECT COUNT(id) AS cpt FROM static_menu WHERE etablissement_id = :etabId",
      {
...
async function getMaxPosition(etablissementId) {
  try {
    const [results, metadata] = await sequelize.query(
      "SELECT MAX(position) AS maxposition FROM static_menu WHERE etablissement_id = :etabId",
      {
        replacements: { etabId: etablissementId },
        type: sequelize.QueryTypes.SELECT,
      }
...
router.post("/", auth, async (req, res, next) => {
  ...
  try {
    const created = await Staticmenu.create({
      etablissement_id,
      nom,
      is_active,
      position,
...
// LIST my menus
router.get(
  "/listetablissementstaticmenus/:etabid(\\d+)",
  auth,
  async (req, res, next) => {
    // verify session
    if (!req.Session) {
...
router.get("/:id(\\d+)", auth, async (req, res, next) => {
  // get menu
  const menu = await Staticmenu.findByPk(req.params.id);
...
router.patch("/setselected/:id(\\d+)", auth, async (req, res, next) => {
  ...
  try {
    ...
    await sequelize.query(
      "UPDATE static_menu SET is_active = 0 WHERE etablissement_id = :etabId",
      {
...
router.patch("/moveup/:id(\\d+)", auth, async (req, res, next) => {
  ...
  try {
    // search next menu
    const nextMenu = await Staticmenu.findOne({
      where: {
...
router.patch("/movedown/:id(\\d+)", auth, async (req, res, next) => {
  ...
  try {
    ...
    const previousMenu = await Staticmenu.findOne({
      where: {
        etablissement_id: menu.etablissement_id,
        position: {
...
router.delete("/:id(\\d+)", auth, async (req, res, next) => {
  ...
  try {
    ...
    if (menu.is_active == 1) {
      const menu2 = await Staticmenu.findOne({
        where: {
          etablissement_id: menu.etablissement_id,
          id: {
